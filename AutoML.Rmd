...
```{r,echo=TRUE}
lmfit_2008<- lm(log(SalePrice) ~ `Year Built`+`Year Remod/Add`+`Gr Liv Area`+`Full Bath`+`Half Bath`+`Bedroom AbvGr`+ `Garage Area`, data = ames_raw_2008)
...
variables <- c("SalePrice", "Year Built", "Year Remod/Add", "Gr Liv Area", "Full Bath", "Half Bath", "Bedroom AbvGr", "Garage Area")
selected_data <- ames_raw_2008[variables]

md.pattern(selected_data)

library(ggplot2)

long_data <- reshape2::melt(selected_data)
...
```{r,echo=TRUE}
lmfit_2008<- lm(log(SalePrice) ~ `Year Built`+`Year Remod/Add`+log(`Gr Liv Area`)+`Full Bath`+`Half Bath`+`Bedroom AbvGr`+ `Garage Area`, data = ames_raw_2008)
